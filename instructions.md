...
_(If you have already created the instance and can SSH into it, skip to Phase 2)._
...
_(Note: At the time of this writing, Qwen 2.5 is the latest stable high-performance iteration widely available on Ollama in 8B/7B class that dominates CPU inference. If `qwen3` is explicitly available in your Ollama registry today, replace with `qwen3:8b`)._
...
_(Press `Ctrl+O`, `Enter`, `Ctrl+X` to save and exit nano)._
...
_Make sure to change `ubuntu` if you are using a different username._
